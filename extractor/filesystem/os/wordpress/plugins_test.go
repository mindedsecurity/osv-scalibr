...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		wantCfg wordpress.Config
	}{
		{
			name: "default",
			cfg:  wordpress.DefaultConfig(),
			wantCfg: wordpress.Config{
...
func TestFileRequired(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "wp-content/plugins/*/test.php file",
			path:             "/wp-content/plugins/*/test.php",
			wantRequired:     true,
		...
		{
			name:             "wp-content/plugins/*/test.php file required if file size < max file size",
			path:             "/wp-content/plugins/*/test.php",
		...
		{
			name:             "wp-content/plugins/*/test.php file not required if file size > max file size",
			path:             "/wp-content/plugins/*/test.php",
			fileSizeBytes:    1000 * units.KiB,
		...
		{
			name:             "wp-content/plugins/*/test.php file required if max file size set to 0",
			path:             "/wp-content/plugins/*/test.php",
			fileSizeBytes:    100 * units.KiB,
			maxFileSizeBytes: 0,
			wantRequired:     true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var e filesystem.Extractor = wordpress.New(wordpress.Config{
				Stats:            collector,
